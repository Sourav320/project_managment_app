class UsersController < ApplicationController
    ...
    def submit_login
        ...
        @has_roles = Role.joins(:users).where(users: { id: @user.id })
        if @has_roles.exists?
            if @user and @user.authenticate(user_params[:password])
                # unless @user.roles.exists?(name: "Super Admin")
                    session[:logged_in] = true
                    session[:email] = user_params[:email]
                    session[:userid] = @user[:userid]
                    session[:id] = @user[:id]
                    # render plain: @user[:id]
                    redirect_to dashboard_users_path(id: @user.id), :flash => { :success => "You are logged in successfully as #{session[:email]}" }
                # else
                    # flash[:error] = "Super Admin cannot log in to user dashboard."
                    # render 'login'
                # end
            else
            ...
            flash[:error] = "Your role is not assign for project. So, not authorized to view this content"
            render 'login'
        end
    ...
    def projects
        @user_id = @user.id
        @project_ids = ProjectUser.where(user_id: @user_id).pluck(:project_id)
        ...
        # @projectss = Project.where(user_id: current_user.id)
